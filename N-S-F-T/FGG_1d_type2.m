function  f = FGG_1d_type2(F,knots,accuracy)
%Description:
...
%% 引用格式 Matthew Ferrara (2021). NUFFT, NFFT, USFFT (https://www.mathworks.com/matlabcentral/fileexchange/25135-nufft-nfft-usfft), MATLAB Central File Exchange. Retrieved July 24, 2021.
%%
...
tau = (pi*M_sp/(N*N*R*(R-0.5)));%Suggested value of tau by Greengard [1]
...
% F=F.*(E_4x)/M;
F=F.*(E_4x);
F=[zeros(0.5*(R-1)*N,1);F;zeros(0.5*(R-1)*N,1)];
% f_tau = fftshift(ifftn(ifftshift(F)));
f_tau = ifftshift(fftn(fftshift(F)))/M_r;
